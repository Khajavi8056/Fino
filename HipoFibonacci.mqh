...
public:
   ...
   bool Create()
   {
      return CreateBackground(m_name + "_Bg", m_offset_x, m_offset_y) &&
             ...
             CreateLabel(m_name + "_Title", "Hipo Fibonacci v1.7.4 - 2025/07/25", m_offset_x + 10, m_offset_y + 5, clrWhite, 11, "Calibri Bold") &&
...
public:
   ...
   bool OnButtonClick(string button, string &command)
   {
      if(ObjectGetInteger(0, button, OBJPROP_STATE))
      {
         ...
         ChartRedraw();
         Sleep(100); // بازگرداندن تأخیر برای بازخورد بصری
...
private:
   ...
   void Log(string message, bool is_error = false)
   {
      if(InpEnableLog)
      {
         string log_message = TimeToString(TimeCurrent(), TIME_DATE | TIME_MINUTES | TIME_SECONDS) + ": MotherFibo: " + message;
         Print(log_message);
         if(is_error)
         {
...
public:
   ...
   bool CheckStructureFailure(double current_price)
   {
      ...
      if(fail_condition)
      {
         Log("ساختار شکست خورد: لنگرگاه مادر سوراخ شد", true);
         if(InpVisualDebug)
         {
...
private:
   ...
   void Log(string message, bool is_error = false)
   {
      if(InpEnableLog)
      {
         string log_message = TimeToString(TimeCurrent(), TIME_DATE | TIME_MINUTES | TIME_SECONDS) + ": ChildFibo: " + message;
         Print(log_message);
         if(is_error)
         {
            int handle = FileOpen(InpLogFilePath, FILE_WRITE | FILE_TXT | FILE_COMMON);
...
public:
   ...
   bool CheckFailure(double current_price)
   {
      ...
      if(fail_condition)
      {
         Log("فرزند " + (StringFind(m_name, "Child1") >= 0 ? "اول" : (m_is_success_child2 ? "دوم (موفق)" : "دوم (ناموفق)")) + " شکست خورد", true);
         if(InpVisualDebug)
         {
   ...
   bool CheckFailureChild2OnNewBar()
   {
      ...
      if(!m_is_success_child2)
      {
         ...
         if(break_condition)
         {
            Log("ساختار شکست خورد: کلوز کندل در سطح " + DoubleToString(max_level, 1) + "%", true);
            if(InpVisualDebug)
            {
               string label_name = "Debug_Label_StructureFail_" + TimeToString(TimeCurrent()) + (m_is_test ? "_Test" : "");
...
private:
   ...
   void Log(string message, bool is_error = false)
   {
      if(InpEnableLog)
      {
         string log_message = TimeToString(TimeCurrent(), TIME_DATE | TIME_MINUTES | TIME_SECONDS) + ": " + m_id + ": " + message;
         Print(log_message);
         if(is_error)
         {
            int handle = FileOpen(InpLogFilePath, FILE_WRITE | FILE_TXT | FILE_COMMON);
...
public:
   ...
   bool Initialize()
   {
      ...
      {
         Log("خطا: نمی‌توان مادر را ایجاد کرد", true);
   ...
   bool UpdateOnTick(double current_price, datetime current_time)
   {
      ...
      else if(m_state == MOTHER_ACTIVE)
      {
         ...
         {
            m_state = FAILED; Log("ساختار شکست خورد", true);
            return false;
         }
         ...
         if(m_mother != NULL && InpMotherFixMode == PRICE_CROSS && m_mother.CheckFixingPriceCross(current_price))
         {
            ...
            {
               Log("خطا: نمی‌توان فرزند اول را ایجاد کرد", true);
      ...
      else if(m_state == CHILD1_ACTIVE)
      {
         ...
         if(m_child1 != NULL && m_child1.CheckFailure(current_price))
         {
            ...
            {
               Log("خطا: نمی‌توان فرزند دوم (ناموفق) را ایجاد کرد", true);
         ...
         else if(m_child1 != NULL && m_child1.UpdateOnTick(current_time))
         {
            if(m_child1.IsFixed() && m_child1.CheckChild1TriggerChild2(current_price))
            {
               ...
               if(m_child2 == NULL || !m_child2.Initialize(current_time))
               {
                  Log("خطا: نمی‌توان فرزند دوم (موفق) را ایجاد کرد", true);
                  delete m_child2; m_child2 = NULL; m_state = FAILED;
                  return false;
               }
               m_state = CHILD2_ACTIVE;
