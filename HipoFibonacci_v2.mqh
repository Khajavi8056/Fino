...
input int InpMaxFractalAttempts = 5;      // <<-- این خاکثر تعداد فراکتال برای بررسی مادر
...
//+------------------------------------------------------------------+
//| کلاس CFamily: مدیریت ساختار فیبوناچی (نسخه بازنویسی شده ۲.۰)     |
//| این کلاس به طور کامل بازنویسی شده است.                           |
//+------------------------------------------------------------------+
class CFamily
{
private:
   ...
   // تابع لاگ‌گیری بدون تغییر باقی می‌ماند
   void Log(string message)
   {
      if(InpEnableLog)
...
public:
   // سازنده کلاس بدون تغییر باقی می‌ماند
...
//--- توابع مدیریت وضعیت و سیگنال
   bool IsActive()
   {
      return m_state != COMPLETED && m_state != FAILED && m_state != SEARCHING;
   }
   ...
   {
      SSignal signal = {"", ""};
      // منطق دریافت سیگنال بدون تغییر باقی می‌ماند، چون فقط وضعیت نهایی را چک می‌کند
      if(m_state == CHILD2_ACTIVE && m_child2 != NULL)
      {
         // ما از قیمت ask برای بررسی سیگنال فروش و از bid برای سیگنال خرید استفاده می‌کنیم
         double check_price = (m_direction == LONG) ? SymbolInfoDouble(_Symbol, SYMBOL_BID) : SymbolInfoDouble(_Symbol, SYMBOL_ASK);
         if(m_child2.CheckSuccessChild2(check_price))
         {
            signal.type = m_direction == LONG ? "Buy" : "Sell";
            signal.id = m_id + "_" + TimeToString(TimeCurrent()) + "_" + (string)m_direction + "_" + (string)m_child2.IsSuccessChild2();
...
//+------------------------------------------------------------------+
//| قلب تپنده جدید: تابع اسکن و ساخت از گذشته                        |
//+------------------------------------------------------------------+
   bool ScanAndBuildFromPast()
   {
      m_state = SEARCHING;
      Log("شروع اسکن برای یافتن ساختار معتبر...");

      // کپی کردن داده‌های قیمتی مورد نیاز برای تحلیل
      MqlRates rates[];
      int lookback = InpFractalLookback + 50; // کمی بیشتر برای اطمینان
      if(Bars(_Symbol, _Period) < lookback) return false;
      if(CopyRates(_Symbol, _Period, 0, lookback, rates) < lookback) return false;
      ArraySetAsSeries(rates, true);

      datetime last_fractal_time = rates[1].time; // شروع جستجو از کندل کامل شده قبلی

      // <<-- مرحله ۱: شکار مادرِ فیکس شده و دست‌نخورده
      for(int i = 0; i < InpMaxFractalAttempts; i++)
      {
         SFractal mother_fractal;
         if(m_direction == LONG)
            m_fractal_finder.FindRecentHigh(last_fractal_time, InpFractalLookback, InpFractalPeers, mother_fractal);
         else
            m_fractal_finder.FindRecentLow(last_fractal_time, InpFractalLookback, InpFractalPeers, mother_fractal);

         if(mother_fractal.time == 0)
         {
            Log("تلاش " + (string)(i + 1) + ": فراکتال دیگری یافت نشد. پایان جستجو.");
            break;
         }

         int fractal_index = iBarShift(_Symbol, _Period, mother_fractal.time);
         int last_fractal_index = iBarShift(_Symbol, _Period, last_fractal_time);
         if(fractal_index < 0 || last_fractal_index < 0) continue;

         double mother_zero_price = 0;
         int mother_zero_index = -1;
         datetime mother_zero_time = 0;

         // پیدا کردن کف لگ
         if(m_direction == LONG)
         {
            mother_zero_price = rates[fractal_index].low;
            mother_zero_index = fractal_index;
            for(int j = fractal_index; j < last_fractal_index; j++) {
               if(rates[j].low < mother_zero_price) {
                  mother_zero_price = rates[j].low;
                  mother_zero_index = j;
               }
            }
         }
         ...
         {
            mother_zero_price = rates[fractal_index].high;
            mother_zero_index = fractal_index;
            for(int j = fractal_index; j < last_fractal_index; j++) {
               if(rates[j].high > mother_zero_price) {
                  mother_zero_price = rates[j].high;
                  mother_zero_index = j;
               }
            }
         }
         mother_zero_time = rates[mother_zero_index].time;

         // <<-- اجرای قانون "حرمت نقطه صفر"
         bool is_zero_violated = false;
         for(int k = 1; k < mother_zero_index; k++)
         {
            bool violation = false;
            if(InpStructureBreakMode == PRICE_CROSS1)
               violation = (m_direction == LONG && rates[k].low <= mother_zero_price) || (m_direction == SHORT && rates[k].high >= mother_zero_price);
            else // CANDLE_CLOSE1
               violation = (m_direction == LONG && rates[k].close <= mother_zero_price) || (m_direction == SHORT && rates[k].close >= mother_zero_price);
            
            if(violation)
            {
               is_zero_violated = true;
               break;
            }
         }
         if(is_zero_violated)
         {
            Log("تلاش " + (string)(i + 1) + ": قانون حرمت نقطه صفر نقض شد. کل جستجو متوقف شد.");
            return false;
         }

         // <<-- بررسی فیکس شدن در گذشته (محاسبه مجازی)
         double level_50 = mother_fractal.price + (mother_zero_price - mother_fractal.price) * 0.5;
         datetime fix_time = 0;
         for(int k = mother_zero_index - 1; k >= 1; k--)
         {
            bool fix_condition = false;
            if(InpMotherFixMode == PRICE_CROSS)
               fix_condition = (m_direction == LONG && rates[k].high >= level_50) || (m_direction == SHORT && rates[k].low <= level_50);
            else // CANDLE_CLOSE
               fix_condition = (m_direction == LONG && rates[k].close >= level_50) || (m_direction == SHORT && rates[k].close <= level_50);
            if(fix_condition)
            {
               fix_time = rates[k].time;
               break;
            }
         }
         ...
         if(fix_time > 0)
         {
            Log("موفقیت! مادر معتبر در " + TimeToString(mother_fractal.time) + " با کف " + TimeToString(mother_zero_time) + " یافت شد.");
            m_mother = new CMotherFibo(m_id + "_Mother", InpMotherColor, InpMotherLevels, m_direction, m_is_test);
            m_mother.SetPoints(mother_zero_time, mother_zero_price, mother_fractal.time, mother_fractal.price);
            m_mother.Draw();
            m_state = MOTHER_ACTIVE;

            SimulateForward(fix_time, rates);
            return true;
         }
         last_fractal_time = mother_fractal.time;
      }
      Log("هیچ مادر فیکس شده و معتبری یافت نشد.");
      return false;
   }

//+------------------------------------------------------------------+
//| تابع شبیه‌سازی از نقطه فیکس شدن مادر تا به امروز (نسخه نهایی)    |
//+------------------------------------------------------------------+
   void SimulateForward(datetime start_time, const MqlRates &rates[])
   {
      int start_index = iBarShift(_Symbol, _Period, start_time);
      if(start_index < 1) start_index = 1;

      // متغیرهای مجازی برای فرزند اول
      double vc1_price100 = 0;
      datetime vc1_time100 = 0;
      int vc1_100_index = -1;

      datetime child1_fix_time = 0;
      datetime child1_breakout_time = 0; // زمانی که سقف مادر را می‌شکند
      
      // <<-- شبیه‌سازی کندل به کندل فرزند اول
      for(int i = start_index; i >= 1; i--)
      {
         if((m_direction == LONG && rates[i].high > vc1_price100) || vc1_price100 == 0)
         {
            vc1_price100 = rates[i].high;
            vc1_time100 = rates[i].time;
            vc1_100_index = i;
         }
         if((m_direction == SHORT && rates[i].low < vc1_price100) || vc1_price100 == 0)
         {
            vc1_price100 = rates[i].low;
            vc1_time100 = rates[i].time;
            vc1_100_index = i;
         }

         double vc1_level_50 = vc1_price100 + (m_mother.GetPrice0() - vc1_price100) * 0.5;
         if((m_direction == LONG && rates[i].low <= vc1_level_50) || (m_direction == SHORT && rates[i].high >= vc1_level_50))
         {
            child1_fix_time = rates[i].time;
            break;
         }

         if((m_direction == LONG && rates[i].high > m_mother.GetPrice100()) || (m_direction == SHORT && rates[i].low < m_mother.GetPrice100()))
         {
            child1_breakout_time = rates[i].time;
            break;
         }
      }

      // اگر سرنوشت فرزند اول مشخص شده بود
      if(child1_fix_time > 0) // فرزند اول موفق
      {
         // <<-- تصحیح باگ منطقی در این خط انجام شد
         m_child1 = new CChildFibo(m_id + "_Child1_Success", InpChild1Color, InpChildLevels, m_mother, false, m_is_test);
         m_child1.SetPoints(m_mother.GetTime0(), m_mother.GetPrice0(), vc1_time100, vc1_price100);
         m_child1.Draw();
         m_state = CHILD1_ACTIVE;

         // پیدا کردن نقطه شکست سقف فرزند اول
         datetime child1_real_breakout_time = 0;
         int fix_index = iBarShift(_Symbol, _Period, child1_fix_time);
         for(int i = fix_index; i >= 1; i--)
         {
            if((m_direction == LONG && rates[i].high > vc1_price100) || (m_direction == SHORT && rates[i].low < vc1_price100))
            {
               child1_real_breakout_time = rates[i].time;
               break;
            }
         }

         if(child1_real_breakout_time > 0)
         {
            double child2_zero_price = 0;
            datetime child2_zero_time = 0;
            int breakout_index = iBarShift(_Symbol, _Period, child1_real_breakout_time);
            
            // <<-- منطق اصلاح شده برای نقطه صفر فرزند دوم
            if(m_direction == LONG)
            {
               child2_zero_price = rates[fix_index].low;
               child2_zero_time = rates[fix_index].time;
               for(int k=fix_index; k>=breakout_index; k--) {
                  if(rates[k].low < child2_zero_price) {
                     child2_zero_price = rates[k].low;
                     child2_zero_time = rates[k].time;
                  }
               }
            }
            else
            {
               child2_zero_price = rates[fix_index].high;
               child2_zero_time = rates[fix_index].time;
               for(int k=fix_index; k>=breakout_index; k--) {
                  if(rates[k].high > child2_zero_price) {
                     child2_zero_price = rates[k].high;
                     child2_zero_time = rates[k].time;
                  }
               }
            }
            
            m_child2 = new CChildFibo(m_id + "_Child2_Success", InpChild2Color, InpChildLevels, m_mother, true, m_is_test);
            // نقطه صد فرزند دوم، بالاترین/پایین‌ترین قیمت از زمان کف جدید تا به امروز است
            double c2_price100 = child2_zero_price;
            datetime c2_time100 = child2_zero_time;
            int child2_start_index = iBarShift(_Symbol, _Period, child2_zero_time);

            for(int k=child2_start_index; k>=1; k--) {
               if(m_direction==LONG && rates[k].high > c2_price100) {c2_price100 = rates[k].high; c2_time100 = rates[k].time; }
               if(m_direction==SHORT && rates[k].low < c2_price100) {c2_price100 = rates[k].low; c2_time100 = rates[k].time; }
            }

            m_child2.SetPoints(child2_zero_time, child2_zero_price, c2_time100, c2_price100);
            m_child2.Draw();
            m_state = CHILD2_ACTIVE;
         }
      }
      else if(child1_breakout_time > 0) // فرزند اول ناموفق
      {
         m_child1 = new CChildFibo(m_id + "_Child1_Failed", InpChild1Color, InpChildLevels, m_mother, false, m_is_test);
         m_child1.SetPoints(m_mother.GetTime0(), m_mother.GetPrice0(), vc1_time100, vc1_price100);
         m_child1.Draw();
         m_state = CHILD1_ACTIVE;

         m_child2 = new CChildFibo(m_id + "_Child2_Failed", InpChild2Color, InpChildLevels, m_mother, false, m_is_test);
         // نقطه صد فرزند دوم، بالاترین/پایین‌ترین قیمت از زمان کف مادر تا به امروز است
         double c2_price100 = m_mother.GetPrice0();
         datetime c2_time100 = m_mother.GetTime0();
         int child2_start_index = iBarShift(_Symbol, _Period, m_mother.GetTime0());

         for(int k=child2_start_index; k>=1; k--) {
            if(m_direction==LONG && rates[k].high > c2_price100) {c2_price100 = rates[k].high; c2_time100 = rates[k].time; }
            if(m_direction==SHORT && rates[k].low < c2_price100) {c2_price100 = rates[k].low; c2_time100 = rates[k].time; }
         }
         m_child2.SetPoints(m_mother.GetTime0(), m_mother.GetPrice0(), c2_time100, c2_price100);
         m_child2.Draw();
         m_state = CHILD2_ACTIVE;
      }
   }
...
//+------------------------------------------------------------------+
//| تابع آپدیت در هر تیک (منطق کاملاً جدید و سبک)                   |
//+------------------------------------------------------------------+
   bool UpdateOnTick(double current_price, datetime current_time)
   {
      if(!IsActive()) return true;

      // <<-- خط اول دفاع: بررسی شکست‌های فوری در حالت لایو
      if(m_mother.CheckStructureFailure(current_price) || m_mother.CheckBreakoutFailure(current_price))
      {
         Log("ساختار در حالت لایو با قیمت " + DoubleToString(current_price, _Digits) + " شکست خورد.");
         m_state = FAILED;
         return false;
      }
      
      // <<-- آپدیت لایو فرزند دوم (اگر وجود دارد)
      if(m_state == CHILD2_ACTIVE && m_child2 != NULL)
      {
          m_child2.UpdateOnTick(current_time);
      }
      // <<-- اگر ساختار هنوز در حال تکمیل شدن است و فرزند دوم ندارد
      else if(m_state == CHILD1_ACTIVE && m_child1 != NULL)
      {
         // اینجا منطق لایو برای تکمیل فرزند اول و ساخت فرزند دوم قرار میگیرد
         // این بخش مشابه منطق SimulateForward است ولی با قیمت لایو
         m_child1.UpdateOnTick(current_time);
         
         if(!m_child1.IsFixed())
         {
            if(m_child1.CheckFixing(current_price))
            {
               // فرزند اول در حالت لایو فیکس شد. حالا باید ببینیم فرزند دوم چی میشه
               Log("فرزند اول در حالت لایو فیکس شد.");
               // برای سادگی، فعلا فرض می‌کنیم که بعد از فیکس شدن، منتظر کندل بعدی برای ساخت فرزند دوم می‌مانیم
               // منطق دقیق ساخت فرزند دوم می‌تواند اینجا اضافه شود.
            }
         }
         
         if(m_child1.CheckChild1TriggerChild2(current_price))
         {
            Log("فرزند دوم در حالت لایو فعال شد.");
            // منطق دقیق ساخت فرزند دوم می‌تواند اینجا اضافه شود.
         }
      }
